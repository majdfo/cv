...
# ----------- Sound Alert -----------
sound_path = "alert_sound.mp3"  # ضع الصوت في نفس مجلد التطبيق
...
# ----------- Analysis Function -----------
alert_played = False  # لمنع تشغيل الصوت لكل إطار

def analyze_image(img, model, model_name, container, live=False):
    global alert_played
    ...
    if distractions:
        ...
        if not live and not alert_played:  # صوت فقط للصور الثابتة
            play_alert_sound()
            alert_played = True
    ...
    if data_file and input_source != "Live Stream 🎥":
        ...
        class StreamProcessor(VideoTransformerBase):
            ...
            def transform(self, frame):
                ...
                # لا صوت في البث المباشر، فقط عرض النتائج
